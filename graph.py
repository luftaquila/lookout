...
def generate_dashboard(input_file_path, output_html_path="analysis_report.html"):
    ...
    hourly_avg = df.groupby('hour')['value'].mean().reindex(range(5, 21), fill_value=0).reset_index()
    
    heatmap_data = df.pivot_table(index='weekday_num', columns='hour', values='value', aggfunc='mean')
    heatmap_data = heatmap_data.reindex(index=range(5), columns=range(5, 21))
    ...
    fig.update_xaxes(title_text="시간", dtick=1, row=1, col=1)
    ...
    fig.update_xaxes(title_text="시간", dtick=2, row=2, col=2)
...
if __name__ == "__main__":
    ...
    generate_dashboard(input_filename, "report.html")
